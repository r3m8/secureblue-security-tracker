...
class CVEComparator:
    ...
    def compare_all_cves(self) -> List[Dict[str, Any]]:
        comparison: List[Dict[str, Any]] = []
        base_path = self.storage.base_path

        for package_name in sorted(os.listdir(base_path)):
            package_path = os.path.join(base_path, package_name)
            if not os.path.isdir(package_path):
                continue

            for cve_id in sorted(os.listdir(package_path)):
                cve_path = os.path.join(package_path, cve_id)
                if not os.path.isdir(cve_path):
                    continue

                debian_data = self.storage.load_debian_cve(package_name, cve_id)
                cve_sec_data = self.storage.load_secureblue_cve(package_name, cve_id)
                secureblue_data = self._get_secureblue_status(
                    package_name, cve_id, cve_sec_data
                )
                ...
                comparison.append(
                    {
                        ...
                        "cve_id": cve_id,
                        "debian": debian_data,
                        "secureblue": secureblue_data,
                    }
                )

        return comparison

    def generate_html_report(self, output_file: str = "index.html") -> None:
        comparison = self.compare_all_cves()
        html_content = self._generate_html(comparison)
        with open(output_file, "w", encoding="utf-8", newline="\n") as f:
            f.write(html_content)
        print(f"HTML report generated : {output_file}")

    def generate_json_data(self, output_file: str = "data.json") -> None:
        comparison = self.compare_all_cves()
        sorted_comparison = sorted(
            comparison,
            key=lambda item: self._parse_cve_id(item["cve_id"]),
            reverse=True,
        )
        ...
        json_data: Dict[str, Any] = {
            "metadata": {
                ...
                "total_cves": len(sorted_comparison),
                "package_count": len({item["package"] for item in sorted_comparison}),
            },
            "cves": [],
        }

        for item in sorted_comparison:
            json_data["cves"].append(
                {
                    "package": item["package"],
                    "cve_id": item["cve_id"],
                    "debian": item["debian"],
                    "secureblue": item["secureblue"],
                }
            )
    ...
    def _parse_cve_id(self, cve_id: str) -> Tuple[int, int]:
        match = re.match(r"CVE-(\d{4})-(\d+)", cve_id)
        if match:
            return int(match.group(1)), int(match.group(2))
        return 0, 0

    def _get_secureblue_status(
        self,
        package_name: str,
        cve_id: str,
        secureblue_data: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        package_secureblue = self.storage.load_package_secureblue(package_name)
        if package_secureblue and "ALL" in package_secureblue:
            stream = package_secureblue["ALL"].get("releases", {}).get("stream", {})
            return {
                "status": stream.get("status", "unverified"),
                "mitigated": stream.get("mitigated", False),
                "comment": stream.get("comment", ""),
            }

        if not secureblue_data or cve_id not in secureblue_data:
            return {"status": "unverified", "mitigated": False, "comment": ""}

        releases = secureblue_data.get(cve_id, {}).get("releases", {})
        if "stream" not in releases:
            return {"status": "unverified", "mitigated": False, "comment": ""}
    ...
    def _generate_html(self, comparison: List[Dict[str, Any]]) -> str:
        debian_open = sum(
            1
            ...
            and any(
                r.get("status") == "open"
                for r in item["debian"].get("releases", {}).values()
            )
        )
        debian_resolved = len(comparison) - debian_open

        secureblue_open = 0
        secureblue_resolved = 0
        secureblue_not_affected = 0
        secureblue_unverified = 0
        secureblue_mitigated = 0

        for item in comparison:
            secureblue_data = item["secureblue"]
            if isinstance(secureblue_data, dict) and "status" in secureblue_data:
                status = secureblue_data.get("status", "unverified")
                if status == "open":
                    secureblue_open += 1
                    if secureblue_data.get("mitigated", False):
                        secureblue_mitigated += 1
                elif status == "resolved":
                    secureblue_resolved += 1
                elif status == "not affected":
                    secureblue_not_affected += 1
                else:
                    secureblue_unverified += 1
            else:
                secureblue_unverified += 1
...
function createRow(item) {
    ...
    tr.innerHTML = `
        ...
        <td><span class="mitigation-indicator ${secureblueInfo.mitigated ? 'mitigation-yes' : 'mitigation-no'}">${secureblueInfo.mitigated ? '✓' : '𐄂'}</span></td>
